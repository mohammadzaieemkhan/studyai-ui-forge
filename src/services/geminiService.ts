...
function createPromptForExamGeneration(params: ExamGenerationParams): string {
  ...
  } else if (params.questionType === "long-answer") {
    prompt += "For each question, provide a comprehensive model answer (multiple paragraphs if necessary) that thoroughly addresses all aspects of the question, and include key points that should be covered. ";
  ...
  try {
    ...
    questionBlocks.filter(block => block.trim().length > 0).forEach((block, index) => {
      ...
      if (questionType === "multiple-choice" || questionType === "mixed") {
        ...
        // For long answer, treat everything after "Model Answer:" or similar as the answer
        const modelAnswerIndex = lines.findIndex(line => /Model Answer:|Sample Answer:|Comprehensive Answer:/i.test(line));
        if (modelAnswerIndex !== -1) {
          answer = lines.slice(modelAnswerIndex + 1).join('\n\n');
        }
        
        // Look for key points that should be addressed
        const keyPointsIndex = lines.findIndex(line => /Key Points:|Important Elements:|Should Include:/i.test(line));
        if (keyPointsIndex !== -1) {
          explanation = lines.slice(keyPointsIndex).join('\n');
        }
      } else {
...
function generateFallbackQuestions(questionType: string): GeneratedQuestion[] {
  return Array.from({ length: 5 }, (_, i) => ({
    ...
    options: questionType === "multiple-choice" || questionType === "mixed" 
      ...
      : undefined,
    answer: questionType === "long-answer"
...
function generateAnswer(questionType: string, index: number): string {
  if (questionType === "multiple-choice" || questionType === "mixed") {
    ...
    return `This is a comprehensive model answer for the ${index + 1}th question. 

In discussing this topic, it's important to first understand the fundamental principles. The concept involves multiple interconnected aspects that should be thoroughly examined.

First, we should consider the theoretical framework that underlies this topic. This includes the historical development of the ideas and how they've evolved over time. Key figures and their contributions should be mentioned.

Second, practical applications demonstrate how these concepts work in real-world scenarios. Several examples can be provided to illustrate the practical significance.

Third, it's essential to analyze the limitations and challenges associated with these principles. No theoretical framework is perfect, and understanding its boundaries is crucial for proper application.

Finally, future directions and emerging trends show how this field continues to develop. Recent research has opened new avenues for exploration that may soon change our understanding.

In conclusion, a thorough understanding requires examining both theoretical foundations and practical implications, while acknowledging limitations and future possibilities.`;
  } else {
...
function generateExplanation(subject: string, topic: string, index: number): string {
  if (index % 3 === 0) {
    return `This explanation discusses the concept of ${topic} in ${subject}. The correct answer demonstrates understanding of the key principles involved. Students should focus on the fundamental relationships between the main variables.`;
  } else if (index % 3 === 1) {
    return `When approaching this ${topic} problem in ${subject}, it's important to apply the correct methodologies. The solution requires careful analysis of the given information and application of relevant formulas.`;
  } else {
    return `This ${subject} question tests understanding of ${topic} at a conceptual level. Students should be able to articulate the core principles and demonstrate how they apply to specific scenarios.`;
  }
}
